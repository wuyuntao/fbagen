...
`;
            ...
            for (let type of this.types) {
                code += this.addAccessor(type, true);
            }

            code += this.endNamespace();

            code += this.beginNamespace("Serialization");

            for (let type of this.types) {
                code += this.addSerializer(type);
            }
...
`;
        ...
        addAccessor(type: TypeStatement, isMutable: boolean): string {
            ...
            code += this.beginType(type.name, isStruct, isMutable);

            for (let field of type.fields) {
                if (!this.isDeprecatedField(field)) {
                    code += this.addTypeField(field, isStruct, isMutable);
                }
            }
        ...
        beginType(typeName: string, isStruct: boolean, isMutable: boolean): string {
            typeName = this.getTypeName(typeName, false);
            return `
    public partial ${isStruct ? 'struct' : 'class'} ${typeName}
    {`;
        }
...
`;
        ...
        beginSerializer(typeName: string, isMutable: boolean): string {
            let accessorName = this.getAccessorName(typeName, isMutable, true);
            let serializerName = this.getSerializerName(typeName, false);
            return `
...
`;
        ...
        addSerializeMethod(type: TypeStatement, isMutable: boolean): string {
            let accessorName = this.getAccessorName(type.name, isMutable, true);
            let code = `
...
`;
        ...
        getTypeName(typeName: string, withNamespace: boolean): string {
            return withNamespace ? `${this.namespace}.${typeName}` : typeName;
        }

        getAccessorName(typeName: string, isMutable: boolean, withNamespace: boolean): string {
            return withNamespace ? `${this.namespace}.${isMutable ? "Mutable" : "Immutable"}.${typeName}` : typeName;
        }

        getSerializerName(typeName: string, withNamespace: boolean): string {
            return withNamespace ? `${this.namespace}.Serialization.${typeName}Serializer` : `${typeName}Serializer`;
        }
